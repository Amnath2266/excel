...
if (isset($_POST['m_name'])) {
    ...
    $upload = (isset($_FILES['m_img']) ? $_FILES['m_img']['name'] : '');
    ...
    if ($upload != '') {
        ...
        if ($typefile == '.jpg' || $typefile == '.jpg' || $typefile == '.png') {
            ...
            if (!$m_username_exist) {
                if (strlen($_POST['m_password']) < 5 || strlen($_POST['m_password']) > 20) {

                    echo '<script>
                        setTimeout(function() {
                            swal({
                                title: "ກະລຸນາໃສ່ລະຫັດຜ່ານໃຫ້ມີຄວາມຍາວລະຫວ່າງ 5-20 ຕົວອັກສອນ",
                                type: "error"
                            }, function() {
